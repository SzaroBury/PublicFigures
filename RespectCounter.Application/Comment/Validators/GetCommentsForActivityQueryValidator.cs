...
{
    public GetCommentsForActivityQueryValidator(IReadOnlyRepository repo, IIdentityService identityService)
    {
        RuleFor(x => x.ActivityId)
            .NotEmpty().WithMessage("ActivityId is required.")
            .MustBeAValidGuid()
            .MustBeAnExistingEntityAsync<GetCommentsForActivityQuery, Domain.Model.Activity>(repo);
